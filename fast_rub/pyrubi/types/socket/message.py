...
class Message:
    ...
    @property
    def forward_object_guid(self) -> str | None:
    ...
    @property
    def event_object_guid(self) -> str | None:
    ...
    async def reply_info(self) -> ReplyInfo | None:
        if not self.reply_message_id:
            return
        result = await self.methods.getMessagesById(
            objectGuid=self.object_guid,
            messageIds=[self.reply_message_id]
        )
    ...
    async def reaction(self, reaction: int | str) -> dict:
    ...
    async def check_join(self, object_guid:str) -> bool | None:
        return await self.methods.checkJoin(objectGuid=object_guid, userGuid=self.author_guid)
    
    
    async def download(self, save: bool = False, save_as: str | None = None) -> dict | None:
